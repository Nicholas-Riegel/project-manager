...
saveBtn.onclick = () => {
    ...
    if (inputTitle.value === null || inputTitle.value === ''){
        ...
        let priority = document.querySelector('select').options[document.querySelector('select').selectedIndex].textContent
        ...
        if (newFormTodos.length > 0){
            newFormTodos.forEach(x => {
                let createdFormTodo = createFormTodo(x.id, x.firstChild.textContent)
...
function setProjectDetailsDiv(){ //in todoList()
    ...
    if (pId === '' || pId == undefined || pArray == []){        
        ...
        if (pSelected != undefined){
            ...
            document.querySelector('#pDescription').textContent = pSelected.description

            document.querySelector('#pDueDate').textContent = pSelected.duedate
            
            if (pSelected.priority !== 'Select'){
                document.querySelector('#pPriority').textContent = pSelected.priority
            }
        }
    }
...
function delTodo(e){
    ...
    pSelected.todo = pSelected.todo.filter( x => x.id != e.target.parentNode.id)
...
// display todos
function displayTodos(){ // this is in todoList()
